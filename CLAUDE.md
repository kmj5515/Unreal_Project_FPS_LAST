...
이 문서는 Claude가 이 저장소에서 코드를 분석·제안·수정할 때 반드시 따라야 하는 프로젝트 지침이다.

## 스킬 사용 규칙 (필수)

- 작업을 시작하기 전에 사용 가능한 스킬 목록을 먼저 확인한다.
- 요청과 조금이라도 관련된 스킬이 있으면 무조건 해당 스킬을 Skill 도구로 호출한 뒤 작업한다.
- 스킬을 직접 흉내 내거나, 스킬 없이 자체 방식으로 처리하지 않는다.
