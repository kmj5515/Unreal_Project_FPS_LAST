...
- 작업을 시작하기 전에 현재 세션에서 사용할 수 있는 스킬을 확인한다.
- 요청 내용과 일치하는 스킬이 있으면 선택 사항이 아니라 필수 절차로 사용하며, 해당 `SKILL.md`를 모두 읽고 지침을 따른다.
- 여러 스킬이 관련되면 요청 범위를 충족하는 최소 조합을 선택하고 적용 순서를 사용자에게 알린다.
- Unreal Engine 작업은 기능 영역에 대응하는 `ue-*` 스킬을 사용하고, 스프레드시트·문서·이미지·프레젠테이션 작업은 각각 대응하는 전용 스킬을 사용한다.
- 산출물을 생성하거나 수정하는 작업은 스킬 문서 확인이나 임시 스크립트 작성에서 끝내지 않는다. 스킬이 지정한 도구로 실제 산출물을 처리하고, 사용자가 요구한 최종 파일과 경로로 `EXPORT` 또는 저장까지 완료해야 한다.
- `EXPORT` 전후로 스킬이 요구하는 내용 검사, 오류 검사, 렌더 또는 미리보기 검증을 수행하며, 최종 산출물이 정상적으로 열리고 요청한 변경을 포함하는지 확인한다.
- 스킬이 지정한 필수 도구나 런타임을 사용할 수 없으면 임의의 대체 수단으로 우회하지 않는다. 차단 원인과 영향 범위를 사용자에게 알리고, 대체 수단 사용에 대한 명시적 승인을 받은 뒤 진행한다.
...
- 이미지 생성 도구로 제작한 UI 텍스처의 최종 결과물은 `LastFPS/Content/Assets/Art/UI/Texture` 디렉터리를 루트로 사용한다.
- 별도 저장 경로가 명시되지 않은 생성 UI 이미지는 다른 `Content/UI` 또는 임시 디렉터리에 두지 않는다.
...
- 데이터 관련 CSV 원본은 `LastFPS/Excel` 디렉터리에 생성하고 관리한다.
- 엑셀 또는 CSV 데이터 수정 요청은 `E:/Unreal_Project_FPS_LAST/LastFPS/Excel`에 있는 기존 원본 파일을 직접 수정한다.
- 스프레드시트 작업 결과를 프로젝트 루트의 `outputs` 디렉터리나 별도의 결과 사본으로 생성하지 않는다.
- 검증용 렌더, 미리보기 및 임시 파일이 필요하면 프로젝트 외부의 임시 경로를 사용하고 작업 완료 후 제거한다.
- 원본을 직접 덮어쓰기 전에 대상 통합 문서, 시트, 열 구조와 기존 서식을 확인하며 요청받지 않은 다른 시트나 CSV는 변경하지 않는다.
