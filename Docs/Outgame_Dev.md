...
> 마지막 업데이트: 2026-06-20 · 기준 브랜치: `main`
...
> **전체 완성도: 약 75%** — 기반/허브/인벤토리/상점/퀘스트 표시/모듈 백엔드+UI/SaveGame 영속화 완료.
> 남은 인스코프: 계승자 관리 화면 · 미션 보드 · 설정 적용 BP. (제작·시즌패스·아르케 성장은 보류/범위 밖)
> 포폴 수직 슬라이스(모듈 UI + 로드아웃 + SaveGame) 기준으로는 **약 95%** (남은 건 빌드/인게임 동작 검증).

> **최근 (06-20)** — SaveGame 영속화 완료(`LastFPSSaveGameSubsystem`, 크레딧·아이템·모듈 장착 디스크 유지) / 모듈 장착 UI 완료 / 캐릭터 선택 카드 클릭·하이라이트. (이전 이력은 git 로그 참조)
...
- [x] ✅ **SaveGame 영속화** (`ULastFPSSaveGame` + `LastFPSSaveGameSubsystem`) — 시작 시 슬롯 로드→Economy(크레딧/보유)·Loadout(모듈 장착) 복원, 변경 델리게이트 구독해 변경마다 즉시 저장. 복원→구독 순서로 재저장 루프 차단. **남음: C++ 빌드 검증 + 인게임 동작 확인(구매→재시작→유지)**
...
## 진행 로드맵
- **다음:** 계승자 관리 화면 (보유 목록 / 스탯 / 스킨 프리뷰)
- **그 다음:** 미션 보드/출격(인게임 핸드오프) · 설정 적용 BP · `/Test/` 경로 정리
...
| SaveGame 영속화 | `LastFPSSaveGameSubsystem` | 시작 시 로드→`RestoreFrom` 복원, 변경 델리게이트 구독→`SaveNow` 즉시 저장 (복원 후 구독으로 루프 차단) |
